...
inline void initializeController() {
    ...
    primary.getTrigger(DIGITAL_UP)
        ...
        ->whileTrue((new Rotate(drivetrainSubsystem, 0.0, false))
                        ->withTimeout(1_s)
                        ->andThen((new Rotate(drivetrainSubsystem, 180_deg, false))->withTimeout(1_s))
                        ->repeatedly());
    primary.getTrigger(DIGITAL_DOWN)
        ->whileTrue((new Sequence({
                         drivetrainSubsystem->pct(0.4, 0.4)
                             ->race(bottomIntakeSubsystem->pctCommand(1.0)
                                        ->with(TopIntakePositionCommand::fromClosePositionCommand(topIntakeSubsystem,
                                                                                                  0.95, 0.0))
                                        ->asProxy())
                             ->withTimeout(100_ms),
                         ...
                         drivetrainSubsystem->pct(-0.2, -0.2)->withTimeout(100_ms),
                         drivetrainSubsystem->pct(-0.2, -0.2)
                             ->race(bottomIntakeSubsystem->pctCommand(1.0)
                                        ->with(TopIntakePositionCommand::fromClosePositionCommand(topIntakeSubsystem,
                                                                                                  0.95, 0.0))
...
inline void initializeCommands() {
    ...
    topIntakeWithEject =
        (new Sequence({topIntakeSubsystem->pctCommand(1.0)->until([]() {
                       ...
                       topIntakeSubsystem->pctCommand(1.0)->until([]() {
                           ...
                           return position > 0.71 &&
                                  position < 0.75; // tune these variables to make ejection work better
                       }),
    ...
    cornerClearIntakeSequence =
        topIntakeWithEject->race(bottomIntakeSubsystem->pctCommand(-1.0)->withTimeout(300_ms)->andThen(
            bottomIntakeSubsystem->pctCommand(1.0)->withTimeout(400_ms)));
