...
inline void initializeCommands() {
    ...
    intakeWithEject =
            (new Sequence({intakeNoEject->until(
                           ...
                           intakeNoEject->until([]() {
                               ...
                               return position > 0.365 &&
                                      position < 0.45; // tune these variables to make ejection work better
                           }),
                           bottomIntakeSubsystem->pctCommand(1.0)->race(
                                   topIntakeSubsystem->pctCommand(-1.0)->withTimeout(0.03_s))}))
                    ->repeatedly();
        ...
        basicLoadLB = new Sequence({
                ...
                new ParallelRaceGroup({
                        ...
                        topIntakeSubsystem->pctCommand(1.0)->until(
                                []() { return topIntakeSubsystem->stalled(300_ms); }),
    ...
    fastLoadLB = new Sequence({
            ...
            new ParallelRaceGroup({bottomIntakeSubsystem->pctCommand(1.0), topIntakeSubsystem->pctCommand(-1.0),
                                   liftSubsystem->positionCommand(CONFIG::WALL_STAKE_LOAD_HEIGHT - 2_deg, 0.0),
                                   new WaitCommand(90_ms)}),
            new ParallelRaceGroup({bottomIntakeSubsystem->pctCommand(1.0), topIntakeSubsystem->pctCommand(1.0),
                                   liftSubsystem->positionCommand(CONFIG::WALL_STAKE_LOAD_HEIGHT - 2_deg, 0.0),
                                   new WaitCommand(160_ms)}),
            new ParallelRaceGroup({bottomIntakeSubsystem->pctCommand(1.0), topIntakeSubsystem->pctCommand(0.0),
                                   liftSubsystem->positionCommand(CONFIG::WALL_STAKE_PRIME_HEIGHT, 0.0),
    ...
    hang = new ParallelCommandGroup(
            {new Sequence({
                     ...
                     new ParallelRaceGroup({drivetrainSubsystem->hangOut(1.0, -5.0_in),
                                            hangSubsystem->levelCommand(false),
                                            liftSubsystem->positionCommand(70_deg, 0.0), new WaitCommand(200_ms)}),
             }),
...
inline void subsystemInit() {
    ...
    drivetrainSubsystem->addLocalizationSensor(
            new Distance(CONFIG::DISTANCE_LEFT_OFFSET, 2388.0 / 2445.0, pros::Distance(4)));
    drivetrainSubsystem->addLocalizationSensor(
            new Distance(CONFIG::DISTANCE_FRONT_OFFSET, 2388.0 / 2428.0, pros::Distance(6)));
    drivetrainSubsystem->addLocalizationSensor(
            new Distance(CONFIG::DISTANCE_RIGHT_OFFSET, 2388.0 / 2415.0, pros::Distance(7)));
    drivetrainSubsystem->addLocalizationSensor(
            new Distance(CONFIG::DISTANCE_BACK_OFFSET, 2388.0 / 2443.0, pros::Distance(10)));
