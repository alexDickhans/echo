...
TrapProfile::Constraints::Constraints(const double max_velocity, const double max_acceleration) :
    maxVelocity(max_velocity), maxAcceleration(max_acceleration) {}

TrapProfile::State::State(double position, double velocity) : position(position), velocity(velocity) {}

bool TrapProfile::shouldFlipAcceleration(State initial, State goal) { return initial.position > goal.position; }

TrapProfile::State TrapProfile::direct(const State &in, const double direction) {
...
TrapProfile::TrapProfile(const Constraints &constraints) : constraints(constraints) {}


inline TrapProfile::State TrapProfile::calculate(const QTime t, const State &initial, const State &end) const {
